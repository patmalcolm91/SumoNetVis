...
attribute for each object type. Each actor is also given an attribute ```sumo_object``` which contains a reference to
the SumoNetVis object that generated it. The actors can then be acted on as desired, for example to create custom
